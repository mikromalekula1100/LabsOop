...
class HexagonValidator : public FigureValidator<T, D>{
...
template <Number T, Number D>
bool HexagonValidator<T, D>::validate(vector<Point<T, D>>& list) { 
    if(list.size() != HexagonValidator<T, D>::numberVertices){
